...
function searchLbc_(sendMail){
  logHeader_("Execution de la recherche");
  var to = ScriptProperties.getProperty(emailPropertyName);
  if(sendMail && (to == "" || to == null) ){
    log_("L'email du destinataire n'est pas définit. Allez dans le menu \"" + appLabel + "\" puis \"" + menuMailSetupLabel + "\".", true);
  } else {
...
function clearLog(maxLogRows) {
  if(maxLogRows == null) {
    logHeader_("Suppression des logs");
    maxLogRows = 0;
  }
  var ss = SpreadsheetApp.getActiveSpreadsheet();
...
function onOpen() {
  ...
  logHeader_("Création du menu et aide");
  var ss = SpreadsheetApp.getActiveSpreadsheet();
...
{
  if(executeWizard){
    logHeader_("Execution du wizard");
    var quest = Browser.msgBox("Voulez-vous exécuter le wizard d'installation ?" , Browser.Buttons.YES_NO);
    if(quest == "yes"){
      if(setupMail()) {
        initSearchId();
        log_("Fin du wizard.\nPensez à ajouter les urls de vos recherches sur leboncoin dans la colonne B.");
      }
    }
  }
...
* Logger
*/
function log_(txt, showMsgBox) {
  Logger.log(txt);
  if(showMsgBox) {
    Browser.msgBox(appLabel, txt, Browser.Buttons.OK);
  }
}

function logHeader_(txt) {
  var header = "#############################";
  header +=  "\n# " + txt + " #";
